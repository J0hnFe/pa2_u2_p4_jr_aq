...
public class EstudianteRepoImpl implements EstudianteRepo {
	...
	public List<EstudianteDTO> seleccionarTodosDTO() {
		TypedQuery<EstudianteDTO> query = this.entityManager.createQuery("SELECT NEW com.example.demo.repository.modelo.dto.EstudianteDTO(e.nombre, e.apellido) FROM Estudiante e ", EstudianteDTO.class);
